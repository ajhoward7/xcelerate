...
"""
This code is used to generate an initial training plan for our user,
using the user preferences (stored as a dict) and
...
def generate_mpw(mileage_baseline, mileage_limit, weeks, preferences):
    ...
    for i in range(weeks):
        ...
        if i % constants.increase_period == 0 \
            and current_mileage < mileage_limit \
                and preferences['runner_type'] == 0:
...
def generate_days_per_week(preferences, weeks):
    """
    Given the preferences dictionary and the number of weeks in the plan,
    this function returns a list of the number of training days per week
    for each week of the training plan.

    Number of days per week initialised and then increased every 2nd week
    until reach maximum.
    ...
    # Handles edge case of previous days per week > max_days
    if initial > max_days:
        initial = max_days
...
def combine_miles_days(miles_per_week, days_per_week,
                       preferences, run_vector, today):
    """
    Take MpW and days per week inputs and output training
    plan appropriate from run vector
    """
    ...
    # Initialise start_of_week
    start_of_week = today - timedelta(days=today.weekday())

    # Days available to run
    ...
    for i in range(len(days_per_week)):
        ...
        if days_this_week <= len(available_days):
            ...
            which_days_this_week = np.sort(np.random.choice(available_days,
                                           days_this_week, False))

        else:
            which_days_this_week = np.sort(np.random.choice(range(7),
                                           days_this_week, False))
        ...
        for j in range(days_this_week):
            ...
            if which_days_this_week[j] == 0:
                ...
                run_date.append(start_of_week + timedelta(days=int(
                                which_days_this_week[j])))
    ...
    # Hack to make sure no 0's
    training_plan['miles'] = [max(run, 1.5) for run in adj_run_vector]
    training_plan['run_day'] = run_day
    training_plan['run_date'] = [this_date for this_date in run_date]
    # training_plan['workout'] = run_workout[:len(run_date)][::-1]
    # training_plan['long_run'] = run_long_run[:len(run_date)][::-1]
    training_plan['week_start'] = \
        training_plan.run_date.apply(lambda x: (x -
                                     timedelta(days=x.weekday())))

    race_day = datetime.strptime(preferences['race_date'], '%Y-%m-%d').date()
    # May want to omit this later
    training_plan = training_plan[training_plan['run_date'] > today]
    ...
    training_plan = \
        training_plan.append({'miles': lib.get_race_distance(preferences),
                              'run_date': race_day,
                              'run_day': race_day.weekday(),
                              'week_start': race_day -
                              timedelta(days=race_day.weekday())
                              },
                             ignore_index=True)
...
def build_plan(user):
    ...
    miles_per_week = generate_mpw(mileage_baseline,
                                  mileage_limit, weeks, preferences)
    assert(len(miles_per_week) == weeks)  # De-bugging
    # Part 5: concatenate Part 3 & 4 to create full plan with run vector

    return combine_miles_days(miles_per_week, days_per_week,
                              preferences, run_vector, date.today())


def generate_plan(user):
    training_plan = build_plan(user)
    training_plan.to_csv('main/users/{}/planned_training.csv'.format(user),
                         index=False)
